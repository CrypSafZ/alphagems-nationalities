'use client';

import { memo } from 'react';
import {
  ComposableMap,
  Geographies,
  Geography,
  ZoomableGroup,
} from 'react-simple-maps';
import { CountryCount } from '@/types';

interface WorldMapProps {
  data: CountryCount[];
}

const geoUrl = 'https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json';

// Map ISO 3166-1 alpha-2 to ISO 3166-1 numeric codes used in TopoJSON
const alpha2ToNumeric: Record<string, string> = {
  AF: '004', AL: '008', DZ: '012', AD: '020', AO: '024', AG: '028', AR: '032',
  AM: '051', AU: '036', AT: '040', AZ: '031', BS: '044', BH: '048', BD: '050',
  BB: '052', BY: '112', BE: '056', BZ: '084', BJ: '204', BT: '064', BO: '068',
  BA: '070', BW: '072', BR: '076', BN: '096', BG: '100', BF: '854', BI: '108',
  CV: '132', KH: '116', CM: '120', CA: '124', CF: '140', TD: '148', CL: '152',
  CN: '156', CO: '170', KM: '174', CG: '178', CD: '180', CR: '188', HR: '191',
  CU: '192', CY: '196', CZ: '203', DK: '208', DJ: '262', DM: '212', DO: '214',
  EC: '218', EG: '818', SV: '222', GQ: '226', ER: '232', EE: '233', SZ: '748',
  ET: '231', FJ: '242', FI: '246', FR: '250', GA: '266', GM: '270', GE: '268',
  DE: '276', GH: '288', GR: '300', GD: '308', GT: '320', GN: '324', GW: '624',
  GY: '328', HT: '332', HN: '340', HU: '348', IS: '352', IN: '356', ID: '360',
  IR: '364', IQ: '368', IE: '372', IL: '376', IT: '380', CI: '384', JM: '388',
  JP: '392', JO: '400', KZ: '398', KE: '404', KI: '296', KW: '414', KG: '417',
  LA: '418', LV: '428', LB: '422', LS: '426', LR: '430', LY: '434', LI: '438',
  LT: '440', LU: '442', MG: '450', MW: '454', MY: '458', MV: '462', ML: '466',
  MT: '470', MH: '584', MR: '478', MU: '480', MX: '484', FM: '583', MD: '498',
  MC: '492', MN: '496', ME: '499', MA: '504', MZ: '508', MM: '104', NA: '516',
  NR: '520', NP: '524', NL: '528', NZ: '554', NI: '558', NE: '562', NG: '566',
  KP: '408', MK: '807', NO: '578', OM: '512', PK: '586', PW: '585', PS: '275',
  PA: '591', PG: '598', PY: '600', PE: '604', PH: '608', PL: '616', PT: '620',
  QA: '634', RO: '642', RU: '643', RW: '646', KN: '659', LC: '662', VC: '670',
  WS: '882', SM: '674', ST: '678', SA: '682', SN: '686', RS: '688', SC: '690',
  SL: '694', SG: '702', SK: '703', SI: '705', SB: '090', SO: '706', ZA: '710',
  KR: '410', SS: '728', ES: '724', LK: '144', SD: '729', SR: '740', SE: '752',
  CH: '756', SY: '760', TW: '158', TJ: '762', TZ: '834', TH: '764', TL: '626',
  TG: '768', TO: '776', TT: '780', TN: '788', TR: '792', TM: '795', TV: '798',
  UG: '800', UA: '804', AE: '784', GB: '826', US: '840', UY: '858', UZ: '860',
  VU: '548', VA: '336', VE: '862', VN: '704', YE: '887', ZM: '894', ZW: '716',
};

function WorldMap({ data }: WorldMapProps) {
  // Create a map of numeric codes to counts
  const countByNumeric: Record<string, number> = {};
  let maxCount = 1;

  data.forEach((item) => {
    const numericCode = alpha2ToNumeric[item.country_code];
    if (numericCode) {
      countByNumeric[numericCode] = item.count;
      if (item.count > maxCount) maxCount = item.count;
    }
  });

  const getColor = (numericCode: string) => {
    const count = countByNumeric[numericCode];
    if (!count) return '#1f2937'; // Dark gray for no submissions

    // Gradient from purple to bright purple based on count
    const intensity = Math.min(count / maxCount, 1);
    const r = Math.round(139 + (167 - 139) * intensity);
    const g = Math.round(92 + (139 - 92) * (1 - intensity));
    const b = Math.round(246 + (250 - 246) * intensity);
    return `rgb(${r}, ${g}, ${b})`;
  };

  return (
    <div className="w-full h-[300px] md:h-[400px]">
      <ComposableMap
        projection="geoMercator"
        projectionConfig={{
          scale: 120,
          center: [0, 30],
        }}
        style={{ width: '100%', height: '100%' }}
      >
        <ZoomableGroup>
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies.map((geo) => {
                const numericCode = geo.id;
                const count = countByNumeric[numericCode] || 0;

                return (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
                    fill={getColor(numericCode)}
                    stroke="#374151"
                    strokeWidth={0.5}
                    style={{
                      default: { outline: 'none' },
                      hover: {
                        fill: count > 0 ? '#c4b5fd' : '#374151',
                        outline: 'none',
                        cursor: count > 0 ? 'pointer' : 'default',
                      },
                      pressed: { outline: 'none' },
                    }}
                  />
                );
              })
            }
          </Geographies>
        </ZoomableGroup>
      </ComposableMap>

      {/* Legend */}
      <div className="flex items-center justify-center gap-4 mt-2 text-xs text-white/60">
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-[#1f2937]" />
          <span>No submissions</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-[#8b5cf6]" />
          <span>Has submissions</span>
        </div>
      </div>
    </div>
  );
}

export default memo(WorldMap);
